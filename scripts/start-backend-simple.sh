...
# Start simplified backend services for local testing without SAGE verification
...
# Start Client Server (without SAGE)
start_service "Client Server" \
    "go run client/main.go --port 8086 --root-url http://localhost:8080"

# Start Root Agent (without verification)
start_service "Root Agent" \
    "go run cli/root/main.go --port 8080 --ordering-url http://localhost:8083 --planning-url http://localhost:8084"
