...
func main() {
	port := flag.Int("port", 18086, "client server port")
	...
	clientKey := flag.String("client-key", filepath.Join("keys", "client.key"), "client key file (DID signer identity)")
	flag.Parse()

	// Load client DID+key from keys/client.key
	clientDID, clientKP, err := loadClientIdentity(*clientKey)
	if err != nil {
		panic(err)
	}

	// A2A client used only when SAGE is ON (API decides per-request)
	...
	// Optional SAGE test/config endpoints
	sh := apihandlers.NewSAGEHandler()
	sh.RegisterRoutes(mux)

	fmt.Printf("Client server listening on :%d (root=%s payment=%s, DID=%s)\n", *port, *root, *payment, clientDID)
